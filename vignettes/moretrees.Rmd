...
```{r tree, eval = TRUE, cache = T, warning = FALSE, fig.height = 3, fig.width = 4, fig.align = 'center'}
library(moretrees)
...
knitr::kable(cbind("group" = est$group, exp(est[ , 2:7]), 
                   "outcomes" = outcome_groups, "n_obs" = est$n_obs), 
             digits = 2)
```
...
```{r ml_est1, eval = TRUE}
est_ml <- mod1$beta_ml
knitr::kable(cbind("group" = est_ml$group, exp(est_ml[ , 2:7]), 
                   "outcomes" = outcome_groups, "n_obs" = est$n_obs), 
             digits = 2)
```
