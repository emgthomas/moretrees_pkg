...
The `moretrees` package fits Multi-Outcome Regression with Tree-structured Shrinkage (MOReTreeS) models to case-crossover and matched case-control data when being a 'case' can mean one of multiple related outcomes.
MOReTreeS models require that these outcomes are related according to a known tree.
For example, the outcomes might be diseases that can be classified using a hierarchical classification that can be expressed as a tree, as in the example below.
...
1. performs `information sharing` across the outcomes by shrinking the coefficient estimates for different outcomes towards each other, with the degree of shrinkage determined by how related those outcomes are on the tree.
...
# Fitting MOReTreeS models using `moretrees()`

## Set up

We use a small simulated dataset, `moretreesCaseCrossover`, which has $n = 1000$ case-crossover pairs, $p = 11$ outcomes, an exposure with dimension $k=2$ and $m = 2$ covariates.
...
```{r tree, eval = FALSE, cache = T, warning = FALSE}
library(moretrees)
...
`moretrees()` will throw an error if the latter isn't true, but it's good practice to double-check before hand.

```{r data, eval = FALSE, cache = T, warning = FALSE}
...
```{r mod1, eval = FALSE}
mod1 <- moretrees(Xcase = Xcase, Xcontrol = Xcontrol,
...
We can print the estimated odds ratios (or rate ratios, for a case-crossover model) for the groups discovered by MOReTreeS.
The table also shows which outcomes belong to each group.

```{r moretrees_est1, eval = FALSE}
est <- mod1$beta_moretrees
outcome_groups <- sapply(est$outcomes, function(x) paste0(x, collapse = ", "))
knitr::kable(cbind("group" = est$group, exp(est[ , 2:7]), "outcomes" = outcome_groups, "n_obs" = est$n_obs), digits = 2)
```
...
```{r ml_est1, eval = FALSE}
est_ml <- mod1$beta_ml
knitr::kable(cbind("group" = est_ml$group, exp(est_ml[ , 2:7]), "outcomes" = outcome_groups, "n_obs" = est$n_obs), digits = 2)
```
...
mod2 <- moretrees(Xcase = Xcase, Xcontrol = Xcontrol,
...
This time, the model ran for more than 11,000 iterations before converging, so it was necessary to increase `max_iter` to accommodate this (the default is `max_iter = 5000`). 

We now print the estimated odds ratios for the discovered groups *adjusted for the covariates in* `W`.

```{r moretrees_est2, eval = FALSE}
est <- mod2$beta_moretrees
outcome_groups <- sapply(est$outcomes, function(x) paste0(x, collapse = ", "))
knitr::kable(cbind("group" = est$group, exp(est[ , 2:7]), "outcomes" = outcome_groups, "n_obs" = est$n_obs), digits = 2)
```

Again, we compare to the group-specific estimates obtained using CLR.

```{r ml_est2, eval = FALSE}
est_ml <- mod2$beta_ml
knitr::kable(cbind("group" = est_ml$group, exp(est_ml[ , 2:7]), "outcomes" = outcome_groups, "n_obs" = est$n_obs), digits = 2)
```
...
When `nrestarts > 1`, `moretrees()` runs the restarts on multiple cores by default.
...
dir.create(log_dir)
mod3 <- moretrees(Xcase = Xcase, Xcontrol = Xcontrol,
                  Wcase = Wcase, Wcontrol = Wcontrol,
                  outcomes = outcomes, tr = tr,
