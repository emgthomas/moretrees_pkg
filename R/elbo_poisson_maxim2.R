...
elbo_poisson_maxim2 <- function(prob, mu, Sigma, tau_t, mu_alpha, tau_t_alpha, # variational params
  ...
  for (g in 1:G) {
    ...
    expected_ss_gamma <- expected_ss_gamma +
      (sum(Sigma[g, ]) + sum(mu[g, ] ^ 2))
  }
  ...
  line6 <- (K * G * (1 + log(2 * pi)) + sum(log(Sigma))) / 2 
  ...
  ELBO <- line1 + line2 + line3 + line4 + line5 + line6 + line8 + line9 
