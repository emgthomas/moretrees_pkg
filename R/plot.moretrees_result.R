...
#' @export
#' @importFrom ggtree %<+%
#' @param res Output from \code{moretrees()}
...
plot.moretrees_result <- function(res,
                                  group.text.size = 4,
  ...
  leaves <- names(igraph::V(res$tr)[igraph::degree(res$tr, mode = "out") == 0])
  groups.df <- data.frame(leaves = leaves, Group = as.factor(res$beta_est$group))
  cols_g <- RColorBrewer::brewer.pal(length(levels(groups.df$Group)), "Set3")
  
  # Plot
  p <- ggtree::ggtree(res$tr, ladderize = F, layout = layout)  %<+% groups.df
  if (horizontal) {
