# ------------------------------------------------------------------------------- #
# -------- ssMOReTreeS for binary outcomes  ------------------------------------- #
# -------- Test code ------------------------------------------------------------ #
# ------------------------------------------------------------------------------- #
# set.seed(98647)
...
# lp2 <- W %*% theta
# for (g in 1:G) {
#   lp2 <- lp2 + dsgn$Xstar[[g]] %*% xi[g, ]
# }
# lp2 <- as.numeric(lp2)
# plot(lp[dsgn$ord], lp2)

# Run algo
mod1 <- spike_and_slab_logistic(dsgn$y, dsgn$Xstar, W, update_hyper = T, update_hyper_freq = 10,
...
for (i in 1:max(groups_est)) {
  ...
  y_i <- y[which_i]
  y_i[y_i == -1] <- 0
  mod_ml <- glm(y_i ~ 0 + X[which_i, ], family = binomial(logit))
