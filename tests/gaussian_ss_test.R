...
# put in list
params <- list(mu=mu,prob=prob,Sigma=Sigma,Sigma_inv=Sigma_inv,Sigma_det=Sigma_det,tau_t=tau_t,rho=rho,sigma2=sigma2,tau=tau)
...
while(abs(ELBO.new-ELBO.old)>tol){
  ...
  params <- update_params_normal(X=X,XtX=XtX,y=y,n=n,K=K,G=G,
                                 ...
                                 Sigma_det=params$Sigma_det,tau_t=params$tau_t,sigma2=params$sigma2,rho=params$rho,
                                 tau=params$tau,update.hyper=update.hyper,update.hyper.last=update.hyper.last)
